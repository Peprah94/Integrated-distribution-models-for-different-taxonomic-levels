# Function to run posterior predictive checks
# Uses the results from the MCMC to performs checks of model fit
...
# Function to run run posterior predictive checks
predChecks <- function(i,
  ...
  #repeating packages for parallelisation
  require(nimble)
  require(MCMCglmm)
  library(parallel)
  library(pbapply)
  ...
  code <- nimbleCode({
    ...
    for(spe.tag in 1:n.species){
      betaSpecies[spe.tag]~ dnorm(muSpecies, var=sigmaSpecies)
      betaLatitude[spe.tag] ~ dnorm(muLatitude, var = sigmaLatitude)
    }
    
    # Intercept to be used for the covariate structure for the group count data
    betaLambda ~ dnorm(mulambda, var = sigmaLambda)
    ...
    # sigmaSpecies ~ dinvgamma(2, 1) #community intercept standard deviation
    # sigmaLatitude ~ dinvgamma(2, 1) #community slope standard deviation
    # sigmaLambda ~ dinvgamma(2, 1) # standard deviation of intercept of GCM under the covariate model
    # sigmaAlphaSpecies ~ dinvgamma(2, 1) #detection prob species random effect standard deviation
    # sigmaAlphaSites ~ dinvgamma(2, 1) #detection prob sites random effect standard deviation
    # sigmaVisits ~ dinvgamma(2, 1) #detection prob visits random effect standard deviation
    
    # detection process species effect
    for(spe.tag in 1:n.species){
      alphaSpecies[spe.tag]~ dnorm(muAlphaSpecies, var = sigmaAlphaSpecies)
    }
    
    # detection process sites effect
    for(site.tag in 1:n.sites){
      alphaSites[site.tag]~ dnorm(muAlphaSites, var = sigmaAlphaSites)
    }
    
    # detection process visit effect
    for(visit.tag in 1:n.visit){
      betaVisits[visit.tag] ~ dnorm(muVisits, var = sigmaVisits)
    }
    ...
    for(site.tag in 1:n.sites){
      for(spe.tag in 1:n.species){
        for(visit.tag in 1:n.visit){
          logit(p.tag[site.tag, spe.tag, visit.tag]) <- alphaSites[site.tag] + alphaSpecies[spe.tag] +  betaVisits[visit.tag]
        }
      }
    }
    ...
    if(method == "covariate"){
      for(site.tag in 1:n.sites){ #loop over sites
        for(spe.tag in 1:n.species){#loop over species
          ...
          muAll[site.tag,spe.tag] <- betaSpecies[spe.tag] + eta.lam[site.tag,spe.tag] + betaLatitude[spe.tag] * latitude[site.tag]  + betaLambda
          muPsi[site.tag,spe.tag] <- betaSpecies[spe.tag] + eta.lam[site.tag,spe.tag] + betaLatitude[spe.tag] * latitude[site.tag]
          muLambda[site.tag,spe.tag] <- eta.lam[site.tag,spe.tag] + betaLatitude[spe.tag] * latitude[site.tag] + betaLambda
    ...
    if(method == "shared"){
      for(site.tag in 1:n.sites){ #loop over sites
        for(spe.tag in 1:n.species){#loop over species
          # for(visit.tag in 1:n.visit){
          muAll[site.tag,spe.tag] <- betaSpecies[spe.tag] + eta.lam[site.tag,spe.tag] + betaLatitude[spe.tag] * latitude[site.tag]
          muPsi[site.tag,spe.tag] <- betaSpecies[spe.tag] + eta.lam[site.tag,spe.tag] + betaLatitude[spe.tag] * latitude[site.tag]
          muLambda[site.tag,spe.tag] <- eta.lam[site.tag,spe.tag] + betaLatitude[spe.tag] * latitude[site.tag] + betaSpecies[spe.tag]
          log(lambda[site.tag, spe.tag]) <- muLambda[site.tag,spe.tag]
          cloglog(psi[site.tag, spe.tag]) <- muPsi[site.tag,spe.tag]
          # }
        }
      ...
      for(site.tag in 1:n.sites){ # loop over sites
        for(spe.tag in 1:n.species){ #loop over species
          for(visit.tag in 1:n.visit){ #loop over visits
            ...
            X[site.tag, spe.tag, visit.tag] ~ dbin(z[site.tag, spe.tag]*p.tag[site.tag,spe.tag, visit.tag], n.replicates)
          }
        }
      }
  ...
  #Data for the model
  idm_data <- list(Y = ceiling(simulations_all[[2]]/data$Nsurveys),
                   X = simulations_all[[1]],
                   latitude = rep(latitude),
                   #Nsurvey = log(data$Nsurveys),
                   R = Rmat,
                   df = df)
  ...
  dataNodesY <- dataNodes[!dataNodes %in% dataNodesX]
  
  #topParams <- mwtc$getNodeNames(stochOnly=TRUE, includeData=FALSE, topOnly=TRUE)
  parentNodes <- mwtc$getParents(dataNodes, stochOnly = TRUE)
  ...
  nSamples <- nrow(samples) # Choose 5000 of them 
  samples <- samples[sample(1:nSamples, n.samples, replace = FALSE),]
  nSamples <- nrow(samples) # check that the new number of samples is equal to the n.samples
  
  # matrix to store statistics of interest
  ppSamples <- matrix(0,
                      nrow = nSamples,
                      ncol = 9)
  ...
  # calculate the test statistic using the original data
  ...
  # extract names of MCMC results to be used for simulation of new datasets
  ...
  postNamesExpanded <- mwtc$expandNodeNames(nodes = postNames, returnScalarComponents = TRUE)
  ...
  for(j in seq_len(nSamples)){
    ...
    #insert MCMC results for MCMC iteration j
    ...
    ppSamples[j, 3] <- devianceX
    ppSamples[j, 4] <- nullDevianceX
    ppSamples[j, 5] <- devianceY
    ppSamples[j, 6] <- nullDevianceY
    ppSamples[j, 7] <- devianceYmean
    ppSamples[j, 8] <- nullDevianceYmean
    ppSamples[j, 9] <- devianceYmean >=nullDevianceYmean
