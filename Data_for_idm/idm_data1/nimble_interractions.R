  ...
  code <- nimbleCode({
    ...
    for(site.tag in 1:n.sites){
      for(spe.tag in 1:n.species){
        ...
        lambda[site.tag, spe.tag] <- -log( 1-psi1[site.tag, spe.tag])+0.000001 
...
#inter_estimates_data <- pblapply(simulations_all[[1]], run_nimble_model, cl=1)
#inter_estimates_data <-run_nimble_model(simulations_all[[1]])
#cl <- makeCluster(2)
#setDefaultCluster(cl)
#setDefaultCluster(cl)
inter_estimates_data <- run_nimble_model(simulations_all[[2]])
#inter_estimates_data <- foreach(i=1:6, .export=c('run_nimble_model'))%dopar% {
#  run_nimble_model(simulations_all[[i]])
#}


save(inter_estimates_data, file="estimate_inter_data.RData")
