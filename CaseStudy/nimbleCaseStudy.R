# This script is used to fit MCMC on the PoMS daataset.
...
run_nimble_model <- function(simulations_all,
  ...
  code <- nimbleCode({
    ...
    for(spe.tag in 1:n.species){
      betaSpecies[spe.tag]~ dnorm(muSpecies, var=sigmaSpecies) #species specific intercept for shared structure of joint likelihood and SOM
      betaLatitude[spe.tag] ~ dnorm(muLatitude, var = sigmaLatitude) # species-specific latitudinal gradient effect for all models
    }
    ...
    #parameter priors
   mulambda ~ dnorm(0, sd = 10) # community intercept of FIT count data for covariate structure
...
#build MCMC and compile
  ...
  #return time taken for model to run
  time_taken <- as.numeric(round(end.time-start_time,2))

  #return summary for all the chains
  estimation <- estimate$summary$all.chains


  return(list(estimation,time_taken, estimate))
