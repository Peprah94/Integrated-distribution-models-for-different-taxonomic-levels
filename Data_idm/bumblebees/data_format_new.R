...
# sites = unique(all_surveys$X1km_square)%>%
#   as.data.frame()%>%
#   arrange()%>%
#   c()
# 
# gr_ref_sites <- gr_ref[gr_ref$grid_ref %in% sites$.,]
# 
# #transforming coordinates
# sps <- SpatialPoints(gr_ref_sites[,c("northing","easting")],
#                      proj4string = CRS("+proj=utm +zone=30 +datum=WGS84 +units=m")
#                      )
# spst <- spTransform(sps, CRS("+proj=longlat +datum=WGS84"))
# 
# gr_ref_sites[,c("long", "lat")] <- coordinates(spst)
# pts <- SpatialPointsDataFrame(coords = gr_ref_sites[,c("long","lat")], data = gr_ref_sites[,1], proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs"))
# 
# r <- getData('worldclim', var='bio', res=10) 
# raster::extract(r, pts, df=T)
