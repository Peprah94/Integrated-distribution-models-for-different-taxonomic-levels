...
run_nimble_model <- function(simulations_all){
  ...
  code <- nimbleCode({
    ...
    for(spe.tag in 1:n.species){
      ...
      alpha0[spe.tag] <- logit(mean.p[spe.tag])
    ...
    for(site.tag in 1:n.sites){
      for(spe.tag in 1:n.species){
        mu[site.tag,spe.tag] <- beta0[spe.tag] +eta.lam[site.tag,spe.tag]
  ...
  idm_inits <- function(){list(beta0= rep(1,const$n.species),
                               beta1= rep(1, const$n.species),#covariate effect
                               #alpha0=rep(1, const$n.species), #detection probability
                               alpha1=rep(1, const$n.species),
  ...
  mcmcconf <- configureMCMC(Cmwtc, monitors = c("beta0",  "alpha0", "pis","CorrIn", "mean.lambda","mean.p"))
  #mcmcconf <- configureMCMC(Cmwtc, monitors = c("beta0", "beta1", "alpha0","alpha1"))
  
  
  Rmcmc <- buildMCMC(mcmcconf, enableWAIC =FALSE)
  ...
  est_fixed <- estimation[((const$n.species*const$n.species)+1):((const$n.species*const$n.species)+(const$n.species*2)),2]
  est_pis <- matrix(estimation[,2][((const$n.species*const$n.species)+((const$n.species)*4)+1) : ((const$n.species*const$n.species)+((const$n.species)*4)+(const$n.sites*const$n.species))], nrow=const$n.sites, ncol=const$n.species, byrow=FALSE)
  return(list(est_pis, est_fixed, estimate))
}
...
species_estimates <- pblapply(simulations_all, run_nimble_model, cl=cl)

save(species_estimates, file="estimate_species_data.RData")
