#Script that hosts functions to format the MCMCoutput from CaseStudy
...
extractParsLower <- function(x){
  sigma <- matrix(x[[3]]$summary$all.chains[ grep("sigma|mu|rho", rownames(x[[3]]$summary$all.chains)), 4],
                  nrow=1,
                  #ncol=2,
                  byrow = T)
  colnames(sigma) <- rownames(x[[3]]$summary$all.chains)[grep("sigma|mu|rho", rownames(x[[3]]$summary$all.chains))]
...
extractParsUpper <- function(x){
  sigma <- matrix(x[[3]]$summary$all.chains[ grep("sigma|mu|rho", rownames(x[[3]]$summary$all.chains)), 5],
                  nrow=1,
                  #ncol=2,
                  byrow = T)
  colnames(sigma) <- rownames(x[[3]]$summary$all.chains)[grep("sigma|mu|rho", rownames(x[[3]]$summary$all.chains))]
