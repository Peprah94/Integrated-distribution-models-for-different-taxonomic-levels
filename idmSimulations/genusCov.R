...
for(iCount in 1:2){
  ...
  rep_estimates <- foreach(iter = seq_along(sim), .packages = c("pbapply", "nimble", "MCMCglmm", "coda", "MCMCpack", "boot", "MASS", "parallel","ggmcmc")) %dopar% {
    tryCatch({ run_nimble_model(sim[[iter]], 
                                model = "IG",
                                covariance_prior = "LV", 
                                method = modelVars[iCount]) }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
  }

  save(rep_estimates, file=paste0("idmSimulations/IG/estimateDataIGSmall",modelVars[iCount],"2.RData"))
  stopCluster(cl)
}
