...
run_nimble_model <- function(simulations_all, # simulated dataset
                             method,# the structure of joint likelihood
                             covariance_prior = "LV", # covariance structure for the interractions; it defaults to multiplicative shrinkage gamma process
                             model # the type of model: IDM, Spe (SOM in main paper), IG (GCM) in main paper
                             ){
  library(pbapply)
  library(nimble)
  library(MCMCglmm)
  ...
  code <- nimbleCode({
    ...
    sigmaSpecies ~ dinvgamma(shape = 2, scale = 1) #community intercept standard deviation
    sigmaLatitude ~ dinvgamma(shape = 2, scale = 1) #community slope standard deviation
    sigmaLambda ~ dinvgamma(shape = 2, scale = 1) # standard deviation of intercept of GCM under the covariate model
    sigmaAlphaSpecies ~ dinvgamma(shape = 2, scale = 1) #detection prob species random effect standard deviation
    sigmaAlphaSites ~ dinvgamma(shape = 2, scale = 1) #detection prob sites random effect standard deviation
    sigmaVisits ~ dinvgamma(shape = 2, scale = 1) #detection prob visits random effect standard deviation
  ...
  return(list(estimation, estimate, N_over_rhat, time_taken))
}
